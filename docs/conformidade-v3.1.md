...
- **K. F2.8 — smoke real do agente leitor** (parcial): **F5.6a entregou o worker real** — processo `jump-worker` consome `jobs.agent` do Redis, delega ao `AgentRunner` (Claude primário → Codex fallback), faz callback HMAC para `POST /internal/agent-results/{run_id}` e bate heartbeat periódico. Pipeline `Redis → Runner → Callback` validado por 27 testes pytest (82% cobertura no worker, runner integralmente mockado). **Bônus do F5.6a:** o bug F2.8 que adiou o smoke no ADR 2026-05-11 ("`Not logged in` em headless apesar do TUI logar com sucesso") foi **diagnosticado e corrigido** — `--bare` no Claude Code v2.1.x desabilita OAuth/keychain (vide `claude --help` do v2.1.141: `"Anthropic auth is strictly ANTHROPIC_API_KEY or apiKeyHelper via --settings (OAuth and keychain are never read)"`). Commit `4c22e53` remove `--bare` de `claude_headless.py` e do probe do `wsl_tmux.py`. Smoke `claude -p '...' --output-format json` agora funciona com OAuth. **F5.6b** fecha o débito K com o smoke real contra `bradesco_sas_databricks.expected.json`: carrega prompt versionado `proposal_reader_v1.md`, baixa proposta do R2, gera `docs/f28-bradesco-baseline-quality.md`. A decisão **F5.6a.Y** (onde o `jump-worker` roda — Windows host vs WSL Linux) é forçada pelo smoke. Ver ADRs `2026-05-13 — F5.6a / Worker real` e dev_notes "Claude Code v2.1.x: --bare desabilita OAuth".
