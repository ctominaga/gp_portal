...
- **F5.2.d — `pytest-cov` subreporta cobertura de funções async.** `portfolio.py` mostra 47% e `scope_changes.py` 50% mesmo com 12 testes do F5.2 commit 5 exercitando explicitamente os endpoints novos (`GET /scope-changes`, `pending_transitions_count` no `/portfolio`). Diagnóstico: o módulo `pytest-cov` precisa de `concurrency = ["thread", "greenlet"]` na configuração para tracear `async def` corretamente sob `pytest-asyncio` + `aiosqlite`. Funções **são** executadas (testes 200 OK), mas as linhas aparecem como missing. Total continua em 72% (≥70% ✅). **Hardening em F6:** adicionar `.coveragerc` com config async e confirmar números reais.
